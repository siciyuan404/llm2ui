...
UI 组件目录，按功能模块划分为 chat（聊天交互）、editor（编辑器）、layout（布局）、preview（预览面板）、settings（设置）、showcase（组件展示）和 ui（基础 UI 组件）七个子目录。
...
| ui/ | 基础 UI 组件：Button、Card、Dialog、Input、Label、Select、Slider、Table、Textarea（shadcn-ui） |
