 ...
 * Property 6: 提示词生成完整性
 * 
 * @module lib/llm/prompt-generator.test
 */
...
import type { ComponentDefinition, PropSchema } from '../core/component-registry';
...
describe('Prompt Generator Theme Isolation', () => {
  /**
   * Property 8: Prompt Generation Theme Isolation
   * 
   * *For any* prompt generation operation, the generated prompt SHALL use 
   * the active theme's templates.
   * 
   * **Feature: multi-theme-system, Property 8: Prompt Generation Theme Isolation**
   * **Validates: Requirements 6.2, 6.3, 6.4**
   */
  it('Property 8: prompt uses active theme templates when ThemeManager is available', async () => {
    // 动态导入以避免循环依赖
    ...
    registry.register({
      name: 'TestButton',
      component: createMockComponent('TestButton'),
      category: 'input',
    ...
    const themeAPrompts = {
      templates: {
        zh: {
          systemIntro: '【主题A】你是一个专业的 UI 生成助手。',
          iconGuidelines: '【主题A图标指南】',
          componentDocs: '【主题A组件文档】',
          positiveExamples: '【主题A正面示例】',
          negativeExamples: '【主题A负面示例】',
          closing: '【主题A结尾】',
        },
        en: {
          systemIntro: '[Theme A] You are a professional UI generation assistant.',
          iconGuidelines: '[Theme A Icon Guidelines]',
          componentDocs: '[Theme A Component Docs]',
          positiveExamples: '[Theme A Positive Examples]',
          negativeExamples: '[Theme A Negative Examples]',
          closing: '[Theme A Closing]',
        },
      },
    };
    ...
    const themeBPrompts = {
      templates: {
        zh: {
          systemIntro: '【主题B】你是一个创意 UI 设计助手。',
          iconGuidelines: '【主题B图标指南】',
          componentDocs: '【主题B组件文档】',
          positiveExamples: '【主题B正面示例】',
          negativeExamples: '【主题B负面示例】',
          closing: '【主题B结尾】',
        },
        en: {
          systemIntro: '[Theme B] You are a creative UI design assistant.',
          iconGuidelines: '[Theme B Icon Guidelines]',
          componentDocs: '[Theme B Component Docs]',
          positiveExamples: '[Theme B Positive Examples]',
          negativeExamples: '[Theme B Negative Examples]',
          closing: '[Theme B Closing]',
        },
      },
    };
    ...
    // 清除缓存
    const { clearPromptCache } = await import('./prompt-generator');
    clearPromptCache();
    
    // 生成提示词（使用 ThemeManager）
    const promptA = generateSystemPrompt({
      language: 'zh',
      includeRelevantExamples: false,
    ...
    // 验证使用了 Theme A 的模板
    expect(promptA).toContain('【主题A】');
    expect(promptA).not.toContain('【主题B】');
    
    // 切换到 Theme B
    themeManager.setActiveTheme('theme-b');
    
    // 生成提示词
    const promptB = generateSystemPrompt({
      language: 'zh',
      includeRelevantExamples: false,
      useThemeManager: true,
      useCache: false,
    ...
    // 验证使用了 Theme B 的模板
    expect(promptB).toContain('【主题B】');
    expect(promptB).not.toContain('【主题A】');
    
    // 清理
    ThemeManager.resetInstance();
  });

  it('Property 8b: prompt falls back to default templates when useThemeManager is false', () => {
    const prompt = generateSystemPrompt({
      language: 'zh',
      includeRelevantExamples: false,
    ...
    // 应该使用默认模板
    expect(prompt).toContain('你是一个 UI 生成助手');
  });
});
